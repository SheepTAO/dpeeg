...
from dpeeg.datasets.base import EEGDataset
from dpeeg.transforms.functions import split_train_test, merge_train_test
from dpeeg.transforms.base import Transforms
...
class Experiment(abc.ABC):
    ...
    def __init__(
        ...
        self.out_folder = (
            os.path.join(os.path.abspath(out_folder), net, exp)
            if out_folder
            else os.path.join(DPEEG_DIR, "out", net, exp)
        )
    ...
    def _trans_dataset(self, trainset: list | tuple, testset: list | tuple) -> tuple:
    ...
    def run(
        ...
        if dataset_name:
            ...
            self.data_folder = os.path.join(self.out_folder, dataset.__class__.__name__)
        else:
        ...
        for sub in dataset.keys():
            ...
            test_acc, test_kappa, test_preds, test_target, sub_exp_res = self._run_sub(
                trainset, testset, cls_name, f"sub{sub}"
            )
            ...
            filer.write(f"Acc = {test_acc*100:.2f}% | Kappa = " f"{test_kappa:.2f}\n\n")
        ...
        filer.write(
            f"Acc = {acc_mean:.2f}%\u00B1{acc_std:.2f} | Kappa = {kappa_mean:.2f}\n"
        )
        save_cm_img(
            preds_metric.compute(),
            target_metric.compute(),
            cls_name,
            os.path.join(self.data_folder, "CM.png"),
        )
...
class KFold(Experiment):
    ...
    def _run_sub(
        ...
        save_cm_img(
            test_preds, test_target, cls_name, os.path.join(sub_folder, f"{sub}_CM.png")
        )

        test_kappa = cohen_kappa(
            test_preds_metric.compute(),
            test_target_metric.compute(),
            task="multiclass",
...
class Holdout(Experiment):
    def __init__(
        ...
        var_check: (
            Literal["train_inacc", "train_loss", "val_inacc", "val_loss"] | None
        ) = None,
        split_val: bool = True,
    ...
    def _run_sub(
        ...
        test_kappa = cohen_kappa(
            test_preds, test_target, task="multiclass", num_classes=len(cls_name)
        )
